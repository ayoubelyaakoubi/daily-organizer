...
export default function Organizer() {
  ...
  const openUpgrade = (reason = null) => { setUpgradeReason(reason); setShowUpgrade(true) }

  const openFriends = () => {
    if (isPremium) setShowFriends(true)
    else openUpgrade('Le partage et le classement entre amis sont réservés aux membres Premium')
  }
        ...
        <AnimatePresence mode="wait">
          ...
          {view === 'stats' && (
            <motion.div key="stats" variants={pageVariants} initial="initial" animate="animate" exit="exit">
              <StatsView year={year} onUpgrade={openUpgrade} />
            </motion.div>
          )}
        </AnimatePresence>
      </main>
